...
class DeliverInvoice extends HandleOrder {
    ...
    public function __construct($DeliverOrderBuilder) {
        $DeliverOrderBuilder->orderType = \Svea\AdminService\AdminServiceRequest::CamelCaseOrderType(\ConfigurationProvider::INVOICE_TYPE);
    ...
    public function prepareRequest() {
        ...
        if ($this->orderBuilder->orderType == \Svea\AdminService\AdminServiceRequest::CamelCaseOrderType(\ConfigurationProvider::INVOICE_TYPE)) {
    ...
    private function validateInvoiceDetails($order, $errors) {
        if (isset($order->orderId) && $order->orderType == \Svea\AdminService\AdminServiceRequest::CamelCaseOrderType(\ConfigurationProvider::INVOICE_TYPE) && isset($order->distributionType) == FALSE) {
    ...
    private function validateOrderRows($order, $errors) {
        if ($order->orderType == \Svea\AdminService\AdminServiceRequest::CamelCaseOrderType(\ConfigurationProvider::INVOICE_TYPE) && empty($order->orderRows) && empty($order->shippingFee) && empty($order->invoiceFee)) {
